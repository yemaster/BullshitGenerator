    ...
    <div id="app">
        <div class="bg">
            <div class="ui container">
                <div class="mainPan" :class="{ totop: Mod }">
                    <h1 v-if="Mod == 0">BullShitGenerator</h1>
                    <div class="mainText">
                        主题：<input type="text" v-model="subject" @change="gener()" @input="tst()">
                        <span v-if="Mod == 1">回车键生成</span>
                    </div>
                    <div class="res">
                        <div class="ui raised segment" v-if="Mod == 2" v-html="passage">
                        </div>
                    </div>
                    <div class="ui divider"></div>
                    <div class="footer">
                        请勿将生成的文章用于正规用途<br>
                        CopyRight &copy; 2021 <a href="https://github.com/vt-dev-team">vt-dev-team</a>. 原版:<a href="https://github.com/menzi11/BullshitGenerator">menzi11</a>
                    </div>
                </div>
            </div>
        </div>
...
ForeWords:[
    ...
    methods: {
        rand () {
            this.peel = ( this.peel * 214013 + 2531011 ) % 2147483648;
            return this.peel / 2147483648;
        },
        getSentence (list) {
            let x = Math.floor( this.rand() * list.length);
            return list[x];
        },
        randInt ( minx = 0, maxx = 100, func = this.rand) {
            let num = func() * (maxx - minx) + minx;
            return num;
        },
        getFamousSaying () {
            let Saying = this.getSentence(this.famousSaying)
            Saying = Saying.replace('曾经说过', this.getSentence(this.ForeWords))
            Saying = Saying.replace('这不禁令我深思', this.getSentence(this.BackWords) )
            return Saying
        },
        getArguments () {
            let Sentence = this.getSentence(this.Arguments);
            Sentence = Sentence.replace(RegExp('主题', 'g'),this.subject);
            return Sentence;
        },
        AddPara (Para){
        ...
        gener () {
            ...
            let sj = _this.subject
            let passage = []
            let paras = ''
            let psglength = 0
            while (psglength < 12000) {
                let rd = this.randInt()
                console.log(rd)
                if (rd < 5 && paras.length > 200) {
                    paras = this.AddPara(paras)
                    passage.push(paras)
                    paras = ''
                }
                else if (rd < 20) {
                    let st = this.getFamousSaying()
                    psglength = psglength + st.length 
                    paras = paras + st
                }
                else {
                    let st = this.getArguments()
                    psglength = psglength + st.length 
                    paras = paras + st 
                }
            }
            paras = this.AddPara(paras)
            passage.push(paras)
            let Title = this.getSentence(this.Title)
            Title = Title.replace(RegExp('主题', 'g'),this.subject);
            let pb = '<div class="typo"><div style="text-align: center;"><h3>' + Title +  '</h3></div><div class="ui divider"></div>' + passage.join('</div><div>') + '</div>';
            this.passage = pb
        }  
    },
    components: {
